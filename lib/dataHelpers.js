...
export function orderVisitCostTotal(o) {
  const visitIds = new Set((o?.visits || []).map((visit) => visit.id));
  const records = new Map();
  for (const record of o?.expenseRecords || []) {
    if (record.visitId && !visitIds.has(record.visitId)) continue;
...
export function orderTechnicianUnpaidCostTotal(o) {
  ...
  if (records.size > 0) {
    return Array.from(records.values())
      .filter((record) => record.type === "technician_fee" && record.isSettled !== true)
      .reduce((sum, record) => sum + (Number(record.amount) || 0), 0);
  }
  return 0;
}
...
export function orderTechnicianFeeBreakdown(order, technicians = []) {
  ...
  for (const record of order?.expenseRecords || []) {
    // 顶层费用若关联的 visit 已被删除，就是孤儿费用，不应继续显示在订单卡片上。
    if (record.visitId && !visitById.has(record.visitId)) continue;
    records.set(record.id, { record, visit: visitById.get(record.visitId) });
  }
  ...
  for (const entry of records.values()) {
    ...
    const current = grouped.get(name) || { name, amount: 0, settled: true, hasPendingAdvance: false, hasPendingMonthly: false, hasPendingUndetermined: false };
    current.amount += Number(record.amount) || 0;
    const recordSettled = record.paymentMethod === "advance"
      ? record.advanceReimbursed === true
      : record.paymentMethod === "monthly_settlement"
        ? record.isSettled === true
        : false;
    current.settled = current.settled && recordSettled;
    if (!recordSettled) {
      if (record.paymentMethod === "advance") current.hasPendingAdvance = true;
      else if (record.paymentMethod === "monthly_settlement") current.hasPendingMonthly = true;
      else current.hasPendingUndetermined = true;
    }
    grouped.set(name, current);
  }
  return Array.from(grouped.values()).map((fee) => ({
    ...fee,
    paymentMethod: fee.hasPendingAdvance ? "advance" : fee.hasPendingMonthly ? "monthly_settlement" : null,
    advanceReimbursed: fee.settled,
  }));
}
...
export function technicianFeeStatusColor(fee) {
  return expenseSettlementMeta(fee).color;
}

export function expenseSettlementMeta(record) {
  if (!record) return { label: "", color: "#718087" };
  if (record?.settled === true || record?.advanceReimbursed === true) {
    return { label: "已结清", color: "#2F7A4F" };
  }
  if (record.paymentMethod === "advance") {
    return record.advanceReimbursed === true
      ? { label: "已结清", color: "#2F7A4F" }
      : { label: "已垫付，待报销", color: "#B5450C" };
  }
  return record.isSettled === true
    ? { label: "已结清", color: "#2F7A4F" }
    : record.paymentMethod === "monthly_settlement"
      ? { label: "月结", color: "#C99A1D" }
...
export const WORKLIST_GROUPS = [
  ...
  { key: "wait", title: "当前负责师傅 · 时间待定", statuses: ["待上门"], matches: (o) => !!o.assignedTechnicianId && !o.expectedVisitTime },
...
export function hasAcceptanceMaterial(order) {
  return Boolean(order?.acceptancePhotoUrl?.trim() || order?.acceptanceSignedPdfUrl?.trim());
}
...
export function getOrderTimeoutReminders(order, now = Date.now()) {
  ...
  const inspectionMaterialsMissing = !hasAcceptanceMaterial(order);
  return {
...
export function orderFromDb(row) {
  return {
    id: row.id,
    store: row.store || row.stores || null,
    ...
    expectedVisitPending: row.expected_visit_pending === true,
    ...
    acceptancePhotoUrl: row.acceptance_photo_url,
    acceptanceSignedPdfUrl: row.acceptance_signed_pdf_url,
...
export function orderToDbPatch(data) {
  ...
  if ("expectedVisitPending" in data) patch.expected_visit_pending = !!data.expectedVisitPending;
  ...
  if ("acceptancePhotoUrl" in data) patch.acceptance_photo_url = data.acceptancePhotoUrl || null;
