...
export function getOrderTimeoutReminders(order, now = Date.now()) {
  ...
  return {
    assignmentOverdue: order?.status === "待派工" && !order?.assignedTechnicianId && isOverdueBy(order?.pendingAssignmentAt, now),
    expectedVisitOverdue: order?.status === "待上门" && !hasVisits && expectedValid && now >= expectedTime,
    visitTimeUndetermined: order?.status === "待上门" && !hasVisits && !expectedValid && isOverdueBy(order?.pendingVisitAt, now),
  };
}
