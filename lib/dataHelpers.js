...
export function getFinanceSummary(orders = [], advances = []) {
  ...
  const unpaidTechnicianOrders = orders.filter((order) => !order.technicianSettled && orderTechnicianCostTotal(order) > 0);
  ...
  return {
    ...
    technicianUnpaidTotal: unpaidTechnicianOrders.reduce((sum, order) => sum + orderTechnicianCostTotal(order), 0),
