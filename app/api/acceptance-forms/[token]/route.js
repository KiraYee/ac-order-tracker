...
export async function GET(request, { params }) {
  try {
    ...
    const { data: form, error } = await supabase.from("acceptance_forms").select("id, order_id, token, filled_pdf_path, signed_pdf_path, status, signed_at, created_at").eq("token", params.token).maybeSingle();
